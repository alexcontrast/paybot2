...
async def handle_admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing_lock and now - float(existing_lock.get("ts", 0)) < 65:
    ...
    try:
        # Лимит ожидания 60 секунд: даём Apps Script время записать статус, но не оставляем карточку висеть навсегда.
        ...
        if request and is_cashbox_archived(request):
            ...
            await safe_edit_text(
                query.message,
                old_text + "\n\n⚠️ Сервер не ответил за 60 секунд. Карточка возвращена как была. Если статус записался позже, бот обновит её через сверку.",
