...
async def ensure_bound(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[Dict[str, Any]]:
    """Fast non-blocking binding check.

    Important: this must never call blocking requests directly from the event loop,
    otherwise /start and menu buttons feel frozen while Apps Script thinks.
    """
    ...
    if cached:
        return cached
    try:
        # Short timeout: if Google is slow, do not make the user stare at a dead bot.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # v166: /start запускает быструю проверку привязки по Telegram ID.
    # Важно: проверка идёт через me_fast — без тяжёлого ensureSetup_ на стороне Apps Script.
    context.user_data.pop("bound_user", None)
    progress_msg = await update.message.reply_text("⏳ Проверяю аккаунт…")

    try:
        result = await api_async("me_fast", {"telegramId": update.effective_user.id}, timeout=10)
        user = result.get("user")
        if user:
            context.user_data["bound_user"] = user
...
async def bind_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Аккаунт привязан. Можно создавать заявки.", reply_markup=MAIN_KEYBOARD)
        return ConversationHandler.END
    except Exception as err:
